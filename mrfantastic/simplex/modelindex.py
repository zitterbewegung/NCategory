...
class PrintIndex(ModelIndex):
    effectived_date = DateField(eval_as='obj.created_at if obj.created_at \
                                         and obj.modified_at > obj.created_at \
                                         else obj.modified_at')
    meta_data = StringField(eval_as='" ".join([fld for fld in [str(obj.created_at), \
                                       str(obj.tags), \
                                       str(obj.price)] if fld])')
